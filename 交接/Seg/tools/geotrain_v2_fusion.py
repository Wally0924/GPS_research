...
# 修復導入錯誤
from engine.geo_v2_fusion import MemoryEnhancedGeoSegformer, create_memory_enhanced_geo_segformer, debug_memory_system
...
class GPSAugmentation:
    """GPS數據增強 - 增加GPS信息的多樣性"""
    def __init__(self, noise_std: float = 0.00001, rotation_angle: float = 0.1):
        self.noise_std = noise_std
        self.rotation_angle = rotation_angle
    
    def __call__(self, gps_coords: torch.Tensor, training: bool = True) -> torch.Tensor:
        if not training:
            return gps_coords
        
        # 添加高斯噪聲
        noise = torch.randn_like(gps_coords) * self.noise_std
        augmented_gps = gps_coords + noise
        
        # 小角度旋轉
        if torch.rand(1) > 0.5:
            angle = (torch.rand(1) - 0.5) * 2 * self.rotation_angle
            cos_a, sin_a = torch.cos(angle), torch.sin(angle)
            
            rotation_matrix = torch.tensor([
                [cos_a, -sin_a],
                [sin_a, cos_a]
            ], device=gps_coords.device)
            
            # 中心化 -> 旋轉 -> 去中心化
            center = gps_coords.mean(dim=0, keepdim=True)
            centered = gps_coords - center
            rotated = torch.matmul(centered, rotation_matrix.T)
            augmented_gps = rotated + center
        
        return augmented_gps
...
def monitor_gps_fusion_effectiveness(model, dataloader, device):
    """監控GPS融合的有效性"""
    model.eval()
    ...
    with torch.no_grad():
        for i, data in enumerate(dataloader):
            ...
            # 有GPS的预测
            outputs_with_gps = model(img, gps, update_memory=False)
            pred_with_gps = outputs_with_gps['segmentation_logits'].argmax(1)
            
            # 无GPS的预测（GPS设为0）
            zero_gps = torch.zeros_like(gps)
            outputs_without_gps = model(img, zero_gps, update_memory=False)
            pred_without_gps = outputs_without_gps['segmentation_logits'].argmax(1)
            
            # 计算IoU
            from engine.metric import fast_hist
            
            for j in range(img.shape[0]):
                # 有GPS的IoU
                hist_with = fast_hist(pred_with_gps[j:j+1], ann[j:j+1], model.num_classes)
                iou_with = torch.diag(hist_with) / (hist_with.sum(1) + hist_with.sum(0) - torch.diag(hist_with) + 1e-8)
                with_gps_ious.append(iou_with.mean().item())
                
                # 无GPS的IoU
                hist_without = fast_hist(pred_without_gps[j:j+1], ann[j:j+1], model.num_classes)
                iou_without = torch.diag(hist_without) / (hist_without.sum(1) + hist_without.sum(0) - torch.diag(hist_without) + 1e-8)
                without_gps_ious.append(iou_without.mean().item())
    
    avg_with_gps = sum(with_gps_ious) / len(with_gps_ious)
    avg_without_gps = sum(without_gps_ious) / len(without_gps_ious)
    improvement = avg_with_gps - avg_without_gps
    
    print(f"📊 GPS融合效果評估:")
    print(f"  有GPS mIoU: {avg_with_gps:.4f}")
    print(f"  無GPS mIoU: {avg_without_gps:.4f}")
    print(f"  GPS提升: {improvement:.4f} ({improvement/avg_without_gps*100:.2f}%)")
    
    return improvement


def parse_args() -> Namespace:
    parser = ArgumentParser(description="Multi-Layer GPS + Memory-Enhanced GeoSegformer Training")
    # 數據路徑
...
def main(args: Namespace):
    ...
    print(f"🚀 多層GPS融合 + 記憶增強版 GeoSegformer 訓練配置:")
    ...
    print(f"✅ 創建多層GPS融合+記憶增強版模型，參數量: {sum(p.numel() for p in model.parameters()) / 1e6:.2f}M")
    ...
    # GPS數據增強器
    gps_augmenter = GPSAugmentation(noise_std=0.00001, rotation_angle=0.05)
    ...
    with Progress() as prog:
        whole_task = prog.add_task("Multi-Layer GPS + Memory Training", total=args.max_epochs)
        ...
        for e in range(start_epoch, args.max_epochs + 1):
            ...
            for batch_idx, data in enumerate(train_dataloader):
                ...
                # GPS數據增強
                if model.training:
                    gps = gps_augmenter(gps, training=True)
            ...
            if e % args.val_interval == 0:
                with torch.no_grad():
                    ...
                    # GPS融合效果監控
                    if e % (args.val_interval * 5) == 0:
                        gps_improvement = monitor_gps_fusion_effectiveness(model, val_dataloader, device)
                        logger.tb_log("GPS/Improvement", gps_improvement, e)
                        
                        if gps_improvement < 0.001:
                            print("⚠️ GPS融合效果較弱，建議調整融合策略")
    ...
    print(f"\n🎉 多層GPS融合 + 記憶增強版 GeoSegformer 訓練完成！")
    print(f"📊 最終記憶庫統計:")
