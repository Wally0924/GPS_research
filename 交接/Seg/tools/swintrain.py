...
from engine.geo_v2_swin import MultiLayerGPSImageEncoder
...
def parse_args() -> Namespace:
    ...
    parser.add_argument("--backbone-type", type=str, default="segformer", choices=["segformer", "swin"])
...
def main(args: Namespace):
    ...
    if args.backbone_type == "swin":
        model = MultiLayerGPSImageEncoder(
            backbone_type="swin",
            swin_model="microsoft/swin-base-patch4-window7-224",
            num_classes=num_categories,  # 這要和你分類數一致
            feature_dim=args.feature_dim,  # 這是融合後的維度
            use_gps_fusion=True,
        ...
        model = MultiLayerGPSImageEncoder(
            backbone_type="segformer",
            segformer_model="nvidia/mit-b0",  # 或你要的型號
