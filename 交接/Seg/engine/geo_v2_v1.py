...
class LocationMemoryBank(nn.Module):
    ...
    def __init__(
        ...
        retrieval_difficulty: float = 0.3,  # 检索难度 0-1
        memory_noise: float = 0.05,         # 记忆噪声强度
        max_memories_per_retrieval: int = 3  # 每次检索最多使用的记忆数
        ...
        # ⭐ 新增属性赋值 - 这个很重要！
        self.retrieval_difficulty = retrieval_difficulty
        ...
        print(f"✅ Enhanced LocationMemoryBank initialized:")
        print(f"  Feature dim: {feature_dim}")
        print(f"  Memory size per location: {memory_size}")
        print(f"  Spatial radius: {spatial_radius}")
        print(f"  Retrieval difficulty: {retrieval_difficulty}")
        print(f"  Memory noise: {memory_noise}")
        print(f"  Max memories per retrieval: {max_memories_per_retrieval}")
    ...
    def update_memory(self, gps_coords: torch.Tensor, features: torch.Tensor, losses: torch.Tensor = None):
        ...
        for i in range(batch_size):
            ...
            if losses is not None:
                importance_score = losses[i].item()  # 困難樣本更重要
            else:
                importance_score = feature_norm  # 或者用特徵範數
            ...
            if len(self.memory_bank[gps_key]['features']) > self.memory_size:
                # 找到最不重要的記憶並移除
                scores = self.memory_bank[gps_key]['importance_scores']
                min_idx = scores.index(min(scores))
                
                self.memory_bank[gps_key]['features'].pop(min_idx)
                self.memory_bank[gps_key]['importance_scores'].pop(min_idx)
    ...
    def retrieve_memory(self, gps_coords: torch.Tensor) -> torch.Tensor:
        ...
        for i in range(batch_size):
            ...
            # 1. 精確匹配，但增加難度
            if random.random() < self.retrieval_difficulty:
                # 完全忘記，不檢索任何記憶
            ...
            if gps_key in self.memory_bank and len(self.memory_bank[gps_key]['features']) > 0:
                available_memories = self.memory_bank[gps_key]['features']
                importance_scores = self.memory_bank[gps_key]['importance_scores']
                
                # 根據重要性選擇記憶，但限制數量
                if len(available_memories) <= self.max_memories_per_retrieval:
                    selected_memories = available_memories[:]
                else:
                    # 按重要性排序，選擇最重要的幾個
                    sorted_indices = sorted(range(len(importance_scores)), 
                                        key=lambda x: importance_scores[x], reverse=True)
                    selected_indices = sorted_indices[:self.max_memories_per_retrieval]
                    selected_memories = [available_memories[idx] for idx in selected_indices]
                
                retrieved_features.extend(selected_memories)
                self.hit_count += 1
                hits_in_batch += 1
            
            # 2. 鄰近位置搜索（如果需要）
            if len(retrieved_features) < 2:
                neighbor_count = 0
                for key, memory in self.memory_bank.items():
                    if key != gps_key and len(memory['features']) > 0 and neighbor_count < 1:
                        ...
                        if distance < self.spatial_radius * 2:
                            retrieved_features.append(memory['features'][-1])
                            neighbor_count += 1
            
            # 3. 聚合和添加噪聲
            if retrieved_features:
                if len(retrieved_features) == 1:
                    aggregated = retrieved_features[0]
                else:
                    # 簡單平均
                    aggregated = torch.stack(retrieved_features).mean(dim=0)
                
                # ⭐ 添加記憶噪聲
                if self.memory_noise > 0:
                    noise = torch.randn_like(aggregated) * self.memory_noise
    ...
    def get_memory_stats(self) -> Dict[str, Any]:
        ...
        for memory in self.memory_bank.values():
            all_scores.extend(memory['importance_scores'])
        avg_importance = sum(all_scores) / len(all_scores) if all_scores else 0
        return {
...
class MemoryEnhancedGeoSegformer(nn.Module):
    ...
    def __init__(
        ...
        retrieval_difficulty: float = 0.3,
        memory_noise: float = 0.05,
        ...
        print(f"  Spatial radius: {spatial_radius}")
        print(f"  Retrieval difficulty: {retrieval_difficulty}")  # ⭐ 新增
        print(f"  Memory noise: {memory_noise}")                  # ⭐ 新增
        print(f"  Max memories per retrieval: {max_memories_per_retrieval}")  # ⭐ 新增
    ...
    def forward(
        ...
        seg_losses: torch.Tensor = None  # ⭐ 新增參數
        ...
        if update_memory and self.training:
            self.memory_bank.update_memory(gps, image_embeddings, seg_losses)
...
def create_memory_enhanced_geo_segformer(
    ...
    # ⭐ 替换记忆库为改进版
    model.memory_bank = LocationMemoryBank(
        feature_dim=feature_dim,
        memory_size=memory_size,
        spatial_radius=spatial_radius,
        save_path=memory_save_path,
