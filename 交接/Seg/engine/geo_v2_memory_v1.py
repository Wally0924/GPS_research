    ...
    def forward(
        ...
        if valid_memory_mask.any():
            ...
            if len(valid_indices) > 0:
                ...
                # ✅ 額外 Debug：打印每張圖的 Attention 平均值
                avg_attn_per_sample = attention_weights.squeeze(-1).mean(dim=-1)  # (valid_batch,)
                for idx, sample_attn in zip(valid_indices.tolist(), avg_attn_per_sample.tolist()):
                    print(f"[Debug] Sample idx {idx} | Attention Mean: {sample_attn:.4f}")
        else:
            attn_weight = 0.0
            print(f"[Debug] No valid memory retrieved for this batch, set memory_weight=0.0")
        ...
        outputs = {
            ...
            'memory_weight': attn_weight  # ✅ 真實注意力權重
        }
