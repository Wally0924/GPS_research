...
class EnhancedLocationEncoder(nn.Module):
    """增强的GPS编码器 - 替换你现有的"""
    def __init__(self, rff_dim: int = 512, output_dim: int = 512, 
                 sigmas: List[float] = [0.0001, 0.001, 0.01], dropout: float = 0.1):
        super().__init__()
        ...
        self.enhanced_mlp = nn.Sequential(
            nn.Linear(rff_dim, rff_dim * 2),
            nn.ReLU(),
            nn.Dropout(dropout),
            nn.Linear(rff_dim * 2, rff_dim),
            nn.ReLU(),
            nn.Dropout(dropout),
            nn.Linear(rff_dim, output_dim),
        )
    ...
    def forward(self, gps: torch.Tensor) -> torch.Tensor:
        ...
        # 上下文调制
        context_modulation = self.context_encoder(rff_features)
        modulated_features = rff_features * (1 + 0.1 * context_modulation)
        
        # 增强MLP处理
        enhanced_features = self.enhanced_mlp(modulated_features)
        normalized_features = self.layer_norm(enhanced_features)
        
        # 重要性权重
        importance = self.importance_predictor(normalized_features)
        
        # 加权输出
        final_features = normalized_features * (0.5 + 0.5 * importance)
...
class ImprovedCrossModalFusion(nn.Module):
    """改进的跨模态融合 - 直接替换你现有的"""
    def __init__(self, feature_dim: int = 512, fusion_method: str = "attention"):
        super().__init__()
        self.feature_dim = feature_dim
        
        # 多尺度GPS感知卷积
        self.gps_aware_convs = nn.ModuleList([
            nn.Sequential(
            ...
            nn.Sequential(
                nn.Conv2d(feature_dim * 2, feature_dim, 5, padding=2),
                nn.BatchNorm2d(feature_dim),
                nn.ReLU()
            ),
            nn.Sequential(
        ...
        self.spatial_gate = nn.Sequential(
            nn.Conv2d(feature_dim * 3, feature_dim, 3, padding=1),
            nn.BatchNorm2d(feature_dim),
            nn.ReLU(),
    ...
    def forward(self, image_features: torch.Tensor, location_embeddings: torch.Tensor) -> torch.Tensor:
        ...
        # 多尺度处理
        multi_outputs = []
        for conv in self.gps_aware_convs:
            combined = torch.cat([image_features, location_map], dim=1)
            output = conv(combined)
            multi_outputs.append(output)
        
        # 连接多尺度输出
        concat_features = torch.cat(multi_outputs, dim=1)
        
        # 空间门控
        gate_weights = self.spatial_gate(concat_features)
        gated_features = concat_features * gate_weights
        
        # 最终融合
        fused = self.final_conv(gated_features)
