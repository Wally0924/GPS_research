...
class MemoryEnhancedGeoSegformer(nn.Module):
    ...
    def forward(
        ...
        Args:
            ...
            ablation_mode: "baseline", "gps_only", "full"
        Returns:
        ...
        if ablation_mode == "baseline":
            # 只使用基礎的 SegFormer，不加入 GPS 信息
            segformer_output = self.image_encoder.segformer(images)
            segmentation_logits = F.interpolate(
                segformer_output.logits, 
                size=images.shape[-2:], 
                mode='bilinear', 
                align_corners=False
            )
            ...
            if return_embeddings:
                ...
                zero_embeddings = torch.zeros(batch_size, 256, device=images.device)
                outputs.update({
                    'image_embeddings': zero_embeddings,
                    'location_embeddings': zero_embeddings
                })
        ...
        # 🆕 消融實驗：GPS編碼但無記憶庫
        if ablation_mode == "gps_only":
            # 直接使用GPS編碼進行跨模態融合，跳過記憶庫
            enhanced_location_embeddings = location_embeddings
...
if __name__ == "__main__":
    ...
    # 測試基準版本
    print(f"\n1️⃣ 測試純 SegFormer 基準...")
    outputs_baseline = model(images, gps, ablation_mode="baseline")
    print(f"  基準分割輸出形狀: {outputs_baseline['segmentation_logits'].shape}")
    print(f"  基準記憶權重: {outputs_baseline['memory_weight']:.4f}")
    
    # 測試GPS編碼但無記憶庫
    print(f"\n2️⃣ 測試GPS編碼但無記憶庫...")
    outputs_gps_only = model(images, gps, ablation_mode="gps_only")
    print(f"  GPS-only分割輸出形狀: {outputs_gps_only['segmentation_logits'].shape}")
    print(f"  GPS-only記憶權重: {outputs_gps_only['memory_weight']:.4f}")
    
    # 測試完整版本
    print(f"\n3️⃣ 測試完整版本（GPS+記憶庫）...")
    outputs_full = model(images, gps, ablation_mode="full", return_embeddings=True, update_memory=True)
    print(f"  完整版分割輸出形狀: {outputs_full['segmentation_logits'].shape}")
    print(f"  完整版記憶權重: {outputs_full['memory_weight']:.4f}")
    print(f"  影像嵌入形狀: {outputs_full['image_embeddings'].shape}")
    print(f"  位置嵌入形狀: {outputs_full['location_embeddings'].shape}")
    
    print(f"\n🎉 消融實驗測試完成！")
    print(f"✅ 所有三種模式都能正常運行")
    print(f"✅ 可以用於對比不同組件的效果")