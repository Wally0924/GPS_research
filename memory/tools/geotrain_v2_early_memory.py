...
def main_training_logic(args: Namespace):
    """主要的訓練邏輯 - 支持記憶庫消融"""
    print("="*60)
    print("[DEBUG-ARGS] Training arguments:")
    print(f"  memory_size = {args.memory_size}")
    print(f"  spatial_radius = {args.spatial_radius}")
    print(f"  gps_norm_method = {args.gps_norm_method}")
    print(f"  memory_warmup_epochs = {args.memory_warmup_epochs}")
    print("="*60)
    image_size = 720, 1280
    crop_size = 320, 320
    stride = 240, 240
    ...
    # 只有記憶庫啟用時才添加記憶庫參數
    if memory_enabled and hasattr(model, 'memory_fusion') and hasattr(model, 'memory_attention'):
