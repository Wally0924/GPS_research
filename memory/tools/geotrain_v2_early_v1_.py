...
def create_training_summary_file(args, logdir: str):
    ...
    summary_data = {
        'model_config': {
            ...
            'ablation_mode': getattr(args, 'ablation_mode', 'full'),  # 🆕 記錄消融模式
        },
        'training_config': {
...
def ensure_memory_bank_availability(model, args, epoch):
    ...
    if epoch == args.memory_warmup_epochs:
        ...
        if hasattr(model, 'save_memory_bank'):
...
def parse_args() -> Namespace:
    parser = ArgumentParser(description="Enhanced Memory-Enhanced GeoSegformer Training with Ablation Study")
    ...
    parser.add_argument("--ablation-mode", type=str, default="full",
                       choices=["baseline", "gps_only", "full"],
                       help="Ablation study mode: baseline (pure SegFormer), gps_only (GPS without memory), full (GPS + memory)")
...
def main_training_logic(args: Namespace):
    ...
    if args.ablation_mode == "full" and args.save_memory_stats:
        memory_save_path = os.path.join(args.logdir, "memory_stats.json")
    ...
    print(f"✅ 創建{'記憶增強版' if args.ablation_mode == 'full' else 'GPS編碼版' if args.ablation_mode == 'gps_only' else '基準版'}模型，參數量: {sum(p.numel() for p in model.parameters()) / 1e6:.2f}M")
    ...
    if args.ablation_mode == "baseline":
        ...
        optimizer = torch.optim.AdamW([
            {"params": model.parameters(), "lr": args.lr_head},
        ])
    else:
        # GPS相關版本的完整優化器
        optimizer = torch.optim.AdamW([
    ...
    # 🆕 記錄訓練配置
    logger.info("Training", f"Starting {args.ablation_mode} mode training with early stopping: {args.early_stop}")
    if args.early_stop:
    ...
    with Progress() as prog:
        whole_task = prog.add_task(f"Training ({args.ablation_mode})", total=args.max_epochs)
        ...
        for e in range(start_epoch, args.max_epochs + 1):
            ...
            for batch_idx, data in enumerate(train_dataloader):
                ...
                if args.ablation_mode == "baseline":
                    # 基準模式：不需要GPS，創建假GPS
                    fake_gps = torch.zeros(img.shape[0], 2, device=device)
                    outputs = model(img, fake_gps, ablation_mode="baseline")
                    ...
                    # 根據消融模式設置參數
                    return_embeddings = args.contrastive_weight > 0
                    update_memory = (args.ablation_mode == "full") and is_warmup
                    
                    outputs = model(img, gps, 
                                  return_embeddings=return_embeddings, 
                                  update_memory=update_memory,
                                  ablation_mode=args.ablation_mode)
                    
                    # 對比學習損失
                    if return_embeddings and 'image_embeddings' in outputs:
                ...
                # 定期輸出記憶庫統計（僅完整模式）
                if args.ablation_mode == "full" and batch_idx % 50 == 0:
            ...
            if e % args.val_interval == 0:
                with torch.no_grad():
                    ...
                    # 記憶庫統計（僅完整模式）
                    if args.ablation_mode == "full" and hasattr(model, 'get_memory_stats'):
                        memory_stats = model.get_memory_stats()
    ...
    print(f"\n🎉 {args.ablation_mode.upper()} 模式 GeoSegformer 訓練完成！")
...
def main(args: Namespace):
    ...
    print(f"\n🧪 執行消融實驗 - 模式: {args.ablation_mode.upper()}")
    print(f"📁 結果將保存到: {args.logdir}")
    
    result = main_training_logic(args)
    print(f"✅ {args.ablation_mode.upper()} 模式訓練完成")
    print(f"最終最佳 {args.monitor}: {result:.4f}")
