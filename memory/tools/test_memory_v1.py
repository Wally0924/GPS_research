...
def run_inference_with_memory(model, dataloader, device, args, memory_enabled=True):
    """🆕 執行帶記憶或不帶記憶的推理（新增輸出每張圖的 image/memory 權重）"""
    ...
    with Progress() as prog:
        with torch.no_grad():
            ...
            for batch_idx, data in enumerate(dataloader):
                ...
                # ✅ 新增：輸出每張圖的 image/memory 權重
                memory_weight = outputs.get('memory_weight', 0)
                img_weight = 1 - memory_weight
                print(f"[{data['filename'][0]}] 🖼️ Image: {img_weight:.2f}, 🧠 Memory: {memory_weight:.2f}")
