...
from engine.geo_v2_memory import MemoryEnhancedGeoSegformer, create_memory_enhanced_geo_segformer
...
class MemoryAwareContrastiveLoss(torch.nn.Module):
    ...
    def forward(self, image_embeds: torch.Tensor, location_embeds: torch.Tensor, gps_coords: torch.Tensor) -> torch.Tensor:
        ...
        # 🔍 添加調試輸出
        debug_enabled = getattr(self, '_debug_enabled', True)
        if debug_enabled:
            print(f"🔍 Debug - Batch size: {batch_size}")
            print(f"🔍 Debug - GPS distances min/max: {gps_distances.min():.8f}/{gps_distances.max():.8f}")
            print(f"🔍 Debug - Spatial threshold: {self.spatial_threshold}")
            print(f"🔍 Debug - GPS coords sample: {gps_coords[:2] if batch_size >= 2 else gps_coords}")
        
        # 檢查是否所有GPS距離都是0（所有GPS相同）
        non_zero_distances = gps_distances[gps_distances > 0]
        if non_zero_distances.numel() == 0:
            if debug_enabled:
                print("⚠️ Warning: 所有GPS距離都是0！回退到標準對比損失")
            ...
            if debug_enabled:
                print(f"🔍 Debug - 標準對比損失: {final_loss.item():.4f}")
                print("🔍 Debug - " + "="*50)
        ...
        for i in range(batch_size):
            ...
            # 🔍 調試每個樣本的負樣本數量
            if debug_enabled:
                print(f"🔍 Debug - Sample {i}: {len(neg_indices)} negative samples out of {batch_size-1}")
                if len(neg_indices) > 0:
                    print(f"🔍 Debug - Sample {i} distances to negatives: {gps_distances[i][neg_indices][:3]}")  # 只顯示前3個
            ...
            if len(neg_indices) > 0:
                ...
                # 🔍 調試單個樣本的損失
                if debug_enabled:
                    print(f"🔍 Debug - Sample {i} loss: {loss.item():.4f}, pos_sim: {pos_sim.item():.4f}")
                
                total_loss += loss
                valid_samples += 1
            else:
                if debug_enabled:
                    print(f"🔍 Debug - Sample {i}: No valid negative samples!")
        
        if valid_samples == 0:
            if debug_enabled:
                print("⚠️ Warning: No valid samples for memory-aware contrastive loss! Using standard contrastive loss.")
            ...
            final_loss = total_loss / valid_samples
        
        # 🔍 調試最終結果
        if debug_enabled:
            print(f"🔍 Debug - Valid samples: {valid_samples}/{batch_size}")
            print(f"🔍 Debug - Final contrastive loss: {final_loss.item():.4f}")
            print("🔍 Debug - " + "="*50)
...
def parse_args() -> Namespace:
    ...
    parser.add_argument("--contrastive-weight", type=float, default=0.7, help="🔧 修改為0.7")
    parser.add_argument("--temperature", type=float, default=0.07)
    parser.add_argument("--spatial-threshold", type=float, default=0.000001, help="🔧 修改為更小的值")
    parser.add_argument("--use-standard-contrastive", action="store_true", default=False, 
                       help="Use standard contrastive loss instead of memory-aware version")
    ...
    parser.add_argument("--no-save-memory-stats", action="store_false", dest="save_memory_stats", default=True,help="Do NOT save memory bank statistics (預設會自動儲存)")
...
def main(args: Namespace):
    ...
    print(f"  空間閾值: {args.spatial_threshold}")
    print(f"  使用標準對比損失: {args.use_standard_contrastive}")
    ...
    # 🔍 添加GPS數據範圍檢查
    print("🔍 檢查實際載入的GPS數據:")
    train_dataloader_temp = train_dataset.get_loader(batch_size=min(4, len(train_dataset)), shuffle=False, num_workers=0)
    sample_batch = next(iter(train_dataloader_temp))
    sample_gps = sample_batch['gps']
    
    print(f"  樣本GPS座標形狀: {sample_gps.shape}")
    print(f"  樣本GPS值: {sample_gps}")
    
    if sample_gps.numel() > 0:
        distances = torch.cdist(sample_gps, sample_gps)
        print(f"  GPS距離矩陣形狀: {distances.shape}")
        print(f"  GPS距離矩陣內容:")
        print(f"    {distances}")
        ...
        if non_zero_distances.numel() > 0:
            print(f"    最小距離 (>0): {non_zero_distances.min():.8f}")
            print(f"    最大距離: {distances.max():.8f}")
            print(f"    平均距離: {non_zero_distances.mean():.8f}")
        else:
            print("    ⚠️ 警告: 所有GPS距離都是0！所有樣本的GPS座標相同！")
            print("    這意味著:")
            print("      1. GPS數據可能有問題")
            print("      2. 或者GPS正規化將所有座標映射到同一點")
            print("      3. 需要檢查原始GPS數據和正規化過程")
        
        print(f"  當前spatial_threshold: {args.spatial_threshold}")
        
        # 檢查有多少樣本對會被認為是負樣本
        far_pairs = (distances > args.spatial_threshold).sum().item()
        total_pairs = distances.numel()
        print(f"  超過閾值的樣本對: {far_pairs}/{total_pairs} ({far_pairs/total_pairs*100:.1f}%)")
        
        # 如果所有距離都是0，建議解決方案
        if non_zero_distances.numel() == 0:
            print("  🔧 建議解決方案:")
            print("    1. 檢查原始GPS數據是否真的不同")
            print("    2. 檢查GPS正規化是否有問題")
            print("    3. 暫時使用標準對比損失: --use-standard-contrastive")
            print("    4. 或者設置更小的spatial_threshold: --spatial-threshold 0")
    
    del train_dataloader_temp, sample_batch
    ...
    if args.use_standard_contrastive:
        print("🔧 使用標準對比損失")
    ...
    with Progress() as prog:
        ...
        for e in range(start_epoch, args.max_epochs + 1):
            ...
            for batch_idx, data in enumerate(train_dataloader):
                ...
                # 總損失（記憶預熱期間減少對比學習權重）
                contrastive_weight = args.contrastive_weight * (0.3 if is_warmup else 1.0)  # 🔧 預熱期改為0.3而不是0.1
                total_loss = (args.seg_weight * seg_loss + 
                             contrastive_weight * contrastive_loss)
                
                # 🔍 在第一個batch顯示詳細信息
                if batch_idx == 0:
                    print(f"\n📊 Epoch {e}, Batch {batch_idx} 詳細信息:")
                    print(f"  分割損失: {seg_loss.item():.4f}")
                    print(f"  對比損失: {contrastive_loss.item() if isinstance(contrastive_loss, torch.Tensor) else contrastive_loss:.4f}")
                    print(f"  對比權重: {contrastive_weight:.4f}")
                    print(f"  總損失: {total_loss.item():.4f}")
                    print(f"  預熱模式: {is_warmup}")
                ...
                train_contrastive_loss += contrastive_loss.item() if isinstance(contrastive_loss, torch.Tensor) else contrastive_loss
                ...
                # 🔧 在前幾個batch只顯示調試信息，避免輸出過多
                if batch_idx >= 2:
                    # 暫時禁用調試輸出
                    if hasattr(contrastive_criterion, 'spatial_threshold'):
                        contrastive_criterion._debug_enabled = False
                ...
                with torch.no_grad():
                    ...
                    for data in val_dataloader:
                        ...
                        if args.use_standard_contrastive:
                            ...
                            # 驗證時不顯示調試信息
                            old_debug = getattr(contrastive_criterion, '_debug_enabled', True)
                            contrastive_criterion._debug_enabled = False
                            contrastive_loss = contrastive_criterion(
                                outputs['image_embeddings'], 
                                outputs['location_embeddings'],
                            ...
                            contrastive_criterion._debug_enabled = old_debug
                        
                        total_loss = (args.seg_weight * seg_loss + 
                                     args.contrastive_weight * contrastive_loss)
                        
                        val_seg_loss += seg_loss.item()
                        val_contrastive_loss += contrastive_loss.item() if isinstance(contrastive_loss, torch.Tensor) else contrastive_loss
