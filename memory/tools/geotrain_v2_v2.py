...
from engine.geo_v2_v2 import MemoryEnhancedGeoSegformer, create_memory_enhanced_geo_segformer,debug_memory_system
...
def parse_args() -> Namespace:
    parser = ArgumentParser(description="Memory-Enhanced GeoSegformer Training (方案C)")
    # 數據路徑
    ...
    # ⭐ 方案C 特有參數 - 選擇性記憶
    parser.add_argument("--use-selective-memory", action="store_true", 
                       help="Enable selective memory")
    parser.add_argument("--difficulty-threshold", type=float, default=0.5, 
                       help="Difficulty threshold for selective memory")
    parser.add_argument("--diversity-threshold", type=float, default=0.7, 
                       help="Diversity threshold for selective memory")
    parser.add_argument("--importance-decay", type=float, default=0.95, 
                       help="Importance decay factor")
    parser.add_argument("--max-memory-age", type=int, default=1000, 
                       help="Maximum memory age")
...
def main(args: Namespace):
    ...
    print(f"🚀 方案C - 選擇性記憶增強版 GeoSegformer 訓練配置:")
    ...
    print(f"  使用選擇性記憶: {args.use_selective_memory}")
    if args.use_selective_memory:
        print(f"  困難度閾值: {args.difficulty_threshold}")
        print(f"  多樣性閾值: {args.diversity_threshold}")
        print(f"  重要性衰減: {args.importance_decay}")
    ...
    with Progress() as prog:
        whole_task = prog.add_task("方案C Training", total=args.max_epochs)
        ...
        for e in range(start_epoch, args.max_epochs + 1):
            ...
            for batch_idx, data in enumerate(train_dataloader):
                ...
                outputs = model(
                    ...
                    update_memory=False,  # 先不更新記憶，計算損失後再更新
                ...
                # 計算單個樣本的分割損失（用於選擇性記憶）
                with torch.no_grad():
                    individual_seg_losses = F.cross_entropy(
                        outputs['segmentation_logits'], ann, reduction='none'
                    ).mean(dim=[1, 2])  # 對H,W維度求平均
                ...
                # ⭐ 第二次前向傳播 - 僅用於更新記憶庫
                if args.use_selective_memory:
                    _ = model(img, gps, return_embeddings=False, update_memory=True, 
                             current_losses=individual_seg_losses)
                else:
                    _ = model(img, gps, return_embeddings=False, update_memory=True)
                ...
                if batch_idx % 50 == 0:
                    ...
                    if args.use_selective_memory:
                        logger.info("SelectiveMemory", 
                                   f"Acceptance Rate: {memory_stats.get('memory_acceptance_rate', 0):.3f}, "
                                   f"Rejected: {memory_stats.get('rejected_memories', 0)}")
            ...
            if args.use_selective_memory:
                memory_stats = model.get_memory_stats()
                logger.tb_log("Memory/AcceptanceRate", memory_stats.get('memory_acceptance_rate', 0), e)
                logger.tb_log("Memory/RejectedMemories", memory_stats.get('rejected_memories', 0), e)
                logger.tb_log("Memory/AvgImportance", memory_stats.get('avg_importance_score', 0), e)
    ...
    print(f"\n🎉 方案C - 選擇性記憶增強版 GeoSegformer 訓練完成！")
    print(f"📊 最終記憶庫統計:")
    ...
    if args.use_selective_memory:
        print(f"  記憶接受率: {final_memory_stats.get('memory_acceptance_rate', 0):.4f}")
        print(f"  被拒絕記憶數: {final_memory_stats.get('rejected_memories', 0)}")
        print(f"  平均重要性評分: {final_memory_stats.get('avg_importance_score', 0):.4f}")
